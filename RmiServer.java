...
public class RmiServer implements Runnable, Closeable, IServer {
	...
	@Override
	public byte [] sendFile(File file) throws Exception {
		if(file != null || file.length() > 0) 
		try {		
			byte[] bFile = Files.readAllBytes(new File(file.toString()).toPath());
			heapSort(bFile, bFile.length);
			return bFile;
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    private void heapSort(byte[] array, int count) {
        heapify(array, count);
    ...
    private void heapify(byte[] array, int count) {
        int start = count / 2 - 1;
    ...
    private void siftDown(byte[] array, int start, int end) {
        int root = start;
